...
void setup() {
	...
	while (file) {
		...
		if (mystring.endsWith(".atp") && mystring.length() < 13
			...
			PRINT("  SIZE: ");
			PRINTLN(file.size());
			scenarios[size_scenario++] = mystring.substring(1,
					mystring.length() - 4);
		}
	...
	xTaskCreatePinnedToCore(task_udp_server, /* Task function. */
	...
	xTaskCreatePinnedToCore(task_sd_reader, /* Task function. */
	...
	for (;;) {
		if (isConnected) {
			...
			if (packetSize) {
				...
				if (len
						>= (OPCODE_LENGTH + 1)&& packetBuffer[len - 1] == OPCODE_EOL_CHAR) {
					packetBuffer[len] = 0;
				} else {
					// Invalid Payload;
					continue;
			...
			while (!buffer_eof() && state == STATE_PLAY) {
				...
				if (c == '!') {
					...
					if (selected_index > -1 && selected_index < (bc_idx - 1)) {
						...
						target_strip =
								req.substring(selected_index + 1).toInt();
						data_payload = true;
					}
				} else {
					bc[bc_idx++] = c;
